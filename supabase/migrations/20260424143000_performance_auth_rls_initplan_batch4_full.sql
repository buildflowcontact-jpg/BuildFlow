...
BEGIN
  ...
  LOOP
    ...
    IF (new_qual IS DISTINCT FROM r.qual_expr) OR (new_check IS DISTINCT FROM r.check_expr) THEN
  ...
  RAISE NOTICE 'Updated % policies by auth wrapper normalization', touched_count;
END $$;
